...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (role === 'ADMIN') {
        toast.success(`Right Credentials`);
        navigate('/AdminDashboard');
      } else {
  ...
  const handleGoogleLogin = async (credentialResponse) => {
    try {
      const { credential } = credentialResponse;
        const response = await axios.post("http://localhost:8080/customers/google-login", {
          ...
          <div>
            ...
            <div className="mt-2">
              <input
                ...
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                className="block w-full rounded-md border px-3 py-2 text-base text-gray-900 placeholder:text-gray-400 focus:outline-indigo-600"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative mt-2">
              <input
                ...
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                className="block w-full rounded-md border px-3 py-2 text-base text-gray-900 placeholder:text-gray-400 focus:outline-indigo-600 pr-10"
              />
              <button
                type="button"
                onClick={() => setShowPassword(!showPassword)}
